...
- used the GloVe language model [@pennington14] that represent 6 bilion words with 300-dimensional vector;
- averaged each dimension whithin each abstract;
- use tSNE algorithm [@maaten08] in order to reduce dimensions to 2 

## The landscape of linguistic research

```{r, out.width='100%'}
df <- read_csv("/home/agricolamz/work/bureaucracy/linguistic_convergency/journals/repo/tsne/2023-05-02.csv")
...
```{r, out.width='100%'}
df |> 
  mutate(target = str_detect(journal, "[Pp]hon"),
...
```{r, out.width='100%'}
df |> 
  mutate(target = str_detect(journal, "[Pp]sych"),
         target_alpha = ifelse(target, 1, 0.2),
